/* ============================================================
   ALATEE PRINTING — script.js  v3
   FIXES:
   - openChat() benar-benar buka halaman chat (display block/none)
   - Gemini model: gemini-2.0-flash (sudah di backend)
   - TTS Web Speech API work + preload voices
   - Loading screen typing animation
   - Count-up 24 Jam berputar terus
   - Tidak ada emoji teks (semua SVG di HTML)
   - Float WA sudah dihapus dari HTML
============================================================ */
'use strict';
...
(function injectBaseStyles() {
  ...
  s.textContent = `
    ...
    .ld-bar{
      ...
      background:linear-gradient(90deg,#3B9BF4,#1A7FE0);
      animation:ldbar 2.2s cubic-bezier(.4,0,.2,1) forwards}
...
function appendMessage(role, text) {
  ...
  if (isUser) {
    avatar.innerHTML = '<div class="msg-avatar-sm-icon">Kk</div>';
  } else {
    avatar.innerHTML = `
      <img src="logo.png" alt="Ayla"
        onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
      <div class="msg-avatar-sm-icon" style="display:none">AP</div>`;
  }
